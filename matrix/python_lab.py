...
three_digits_set = { d1*base**2 + d2*base + d3 for d1 in digits for d2 in digits for d3 in digits }
...
## Task 15
exclude_zero_list = [ (i, j, k) for i in S for j in S for k in S if i + j + k == 0 and not (0==i and  0==j and 0==k) ] 

## Task 16
first_of_tuples_list = [ (i, j, k) for i in S for j in S for k in S if i + j + k == 0 and not (0==i and  0==j and 0==k) ] [0]

## Task 17
L1 = [1,2,2,4] # <-- want len(L1) != len(list(set(L1)))
L2 = [4,2,6] # <-- same len(L2) == len(list(set(L2))) but L2 != list(set(L2))
...
## Task 24
D = {'red','white','blue'}
identity_dict = {k:v for (k,v) in zip(range(len(D)),D)}
...
listdict2dict = { names[ii]:salary for (ii,salary) in d.items() }
